...
- **AsyncAgentRuntime** is the execution boundary HTTP and evaluation still call. LangGraph owns **transitions** (`route_node` → `answer_node` | `tool_node` → `verify_node` → `answer_node` | `recovery_node`). Nodes wrap `Router`, tools, `ToolVerifier`, `RecoveryPolicy`, and answer generation; they do not reimplement them. `build_runtime()` selects `AgentRouter` or `LlmAgentRouter` from `ROUTER_MODE` (default `keyword`). An explicit `router=` argument still overrides config.
