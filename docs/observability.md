...
Step events (`run_started`, `route_selected`, `tool_started`, `tool_completed`, `tool_failed`, `verification_completed`, `recovery_decision`, `run_completed`, `run_failed`) are recorded on the in-memory trace in order. Failed tool events may include an `error` code (`not_found`, `cross_tenant`, `wrong_property`, `missing_policy`, …). They do not include `tenant_id` or `property_id`. Logs include `event_names` and `event_count`. Events are not persisted and are not returned by `GET /runs/{run_id}`.
